        ...
        <div class="panel-body">
            ...
            <form name="fileIn" method="post" enctype="multipart/form-data" >
                <div class="form-group form-inline">
                 ...
                 <div class="form-group form-inline">
                    <label for="title">เลขหนังสือ:</label>
                    <input class="form-control" type="text" name="title" size="100%" placeholder="ใส่ชื่อเอกสาร" required="">
                </div>
                <div class="form-group form-inline">
                    <label for="book_no">เลขหนังสือ</label>
                    <input class="form-control" type="text" name="book_no" size="100%" placeholder="ตัวอย่าง พง0017.2/ว1234" required>
                </div>
                <div class="form-group form-inline">
...
?>
	...
	?>
                        <div class="form-group form-inline">
                            <label for="fileupload">ไฟล์แนบ</label><a href="<?php print $link_file;
...
$date=date('Y-m-d');
if(isset($_POST['send'])){
	//ต	รวจสอบการกดปุ่ม send  จากส่งเอกสารภายใน
		$title=$_POST['title'];
	$detail=$_POST['detail'];
	...
	if($toAll==1){
		//ก		รณีส่งเอกสารถึงทุกหน่วยภายใน
				if($cid && $link_file<>null){
			//เ			ป็นนการตรวจสอบว่าเป็นการส่งเอกสารผ่านทางระบบส่งหนังสือโดยตรงหรือไม่
			...
			//ก			รณีส่งเอกสารโดยไม่มีการออกเลข
						$sql="INSERT INTO paper(title,detail,file,postdate,u_id,sec_id,insite,dep_id,book_no)
	...
	elseif($toSome==2){
		...
		$result=dbQuery($sql);
		$lastid=  dbInsertId();
		//ค		นหาเลขระเบียนล่าสุด
				if(!$result){
			echo 'SQL Error';
		}
		else{
			$sendto=$_POST['toSomeUser'];
			//่			ส่วนการเก็บค่า id จาก textbox
						$sendto=substr($sendto, 1);
			$c=explode("|", $sendto);
			for ($i=0;$i<count($c);$i++){
				$sql="SELECT  u.u_id,u.level_id,s.sec_id,d.dep_id FROM user u 
                        INNER JOIN section s ON s.sec_id=u.sec_id
                        INNER JOIN depart d  ON d.dep_id=u.dep_id
                        WHERE s.sec_id=$c[$i] 
                        AND u.keyman=1 ";
				//ส				ารบรรณประจำกลุ่มหรือผู้มีสิทธิ์เท่านั้น
		...
		//i		f ตรวจสอบว่ามีการบันทึกข้อมูลในตารางหลักแหล้วหรือไม่
	}
	//i	f ตรวจสอบว่าส่งถึงใคร
}
//if isset
